...
io.on("connection", async (socket) => {
  ...
  socket.on("start", (id) => {
    ...
    while (i < searching.length) {
      ...
      if (peer.id !== id) {
        ...
        io.of("/")
          .to(roomName)
          .emit("chatStart", "Bạn đang trò chuyện với một người lạ");
  ...
  socket.on("stop", () => {
    ...
    peer.emit("strangerDisconnected", "Đối phương đã thoát!");

    socket.emit("endChat", "Bạn đã ngắt kết nối!");
  ...
  socket.on("disconnecting", async () => {
    ...
    if (roomName) {
      io.of("/").to(roomName).emit("goodBye", "Đối phương đã thoát");
